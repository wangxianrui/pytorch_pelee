...
class StemBlock(nn.Module):
    def __init__(self, inp=3, num_init_features=32):
        ...
        self.stem_2a = Conv_bn_relu(
            num_init_features, int(num_init_features / 2), 1, 1, 0)

        self.stem_2b = Conv_bn_relu(
            int(num_init_features / 2), num_init_features, 3, 2, 1)

        self.stem_2p = nn.MaxPool2d(kernel_size=2, stride=2)

        self.stem_3 = Conv_bn_relu(
            num_init_features * 2, num_init_features, 1, 1, 0)
...
class DenseBlock(nn.Module):
    def __init__(self, inp, inter_channel, growth_rate):
        print(inp, inter_channel, growth_rate)
    ...
    def __init__(self, num_classes=1000, num_init_features=32, growthRate=32, nDenseBlocks=[3, 4, 8, 6],
        ...
        for i, b_w in enumerate(bottleneck_width):
            ...
            if i == 0:
                ...
                total_filter.append(
                    total_filter[i - 1] + growthRate * nDenseBlocks[i])
    ...
    def _make_dense_transition(self, dense_inp, total_filter, inter_channel, ndenseblocks, with_pooling=True):
        ...
        for i in range(ndenseblocks):
            layers.append(DenseBlock(
                dense_inp, inter_channel, self.half_growth_rate))
            dense_inp += self.half_growth_rate * 2
            # print(dense_inp, inter_channel, self.half_growth_rate)

        # Transition Layer without Compression
        layers.append(TransitionBlock(dense_inp, total_filter, with_pooling))
        # print(dense_inp, total_filter, with_pooling)
        # exit(0)
        if with_pooling:
